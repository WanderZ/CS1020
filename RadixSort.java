...
class RadixSort {
    public static void main (String args[]) {
        ...
        int[] a = {123, 2154, 222, 4, 283, 1560, 1061, 2, 22, 15};
    ...
    // @overriding method
    private void sort (int[] a) {	sort (a, 10, 4); }  // 8 4-digit numerics
    ...
    private void sort (int[] a, int n, int d) {
        for (int j = d; j > 0; j --) {
            ...
            // Queue<Integer> q = {q0, q1, q2, q3, q4, q5, q6, q7, q8, q9}; // Error!! /// That is how you define a int array, not a queue...
            */
            ...
            for (int i = 0; i < n; i ++) {
                ...
                ((Queue<Integer>)p[k]).offer(a[i]); // q[k].add(a[i]);
            }
            ...
            for (int i = 0; i < 10; i ++) {
                while (p[i].peek() != null) {
                    a[position ++] = (Integer) p[i].poll(); // int = (int) Integer
                }
            }
        } 
    }
